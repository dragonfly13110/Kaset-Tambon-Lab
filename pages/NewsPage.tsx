...
const NewsPage: React.FC<NewsPageProps> = ({ onNavigateHome }) => {
  ...
  const renderContent = () => {
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center gap-3 text-center min-h-[50vh]">
          ...
          <p className="text-sm text-slate-500">กำลังดึงข้อมูลข่าวสารล่าสุด...</p>
        </div>
      );
    }
    ...
    if (error) {
      return (
        <div className="flex flex-col items-center justify-center gap-3 text-center min-h-[50vh] rounded-2xl bg-rose-50 p-4">
        ...
        {articles.map((article) => (
          ...
          >
            <a
              ...
              className="flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:border-agri-200 hover:shadow-lg"
              ...
              <div className="flex flex-1 flex-col justify-between p-5">
                <div>
                  <div className="mb-3 flex items-center justify-between text-[11px]">
                    <span className="inline-block rounded-full bg-agri-50 px-2 py-0.5 font-medium text-agri-700 ring-1 ring-inset ring-agri-600/20">
                      {article.source}
                    </span>
                    <span className="text-slate-400">{formatDate(article.pubDate)}</span>
                  </div>
                  <h3 className="text-base font-semibold text-slate-900 line-clamp-3">{article.title}</h3>
                  <p className="mt-2 text-xs leading-relaxed text-slate-600 line-clamp-3">{stripHtml(article.description)}</p>
                </div>
                <div className="mt-4 flex items-center gap-2 text-xs font-semibold text-agri-600 transition-colors group-hover:text-agri-700">
